...
*values* it consumes are quantisation-aware. The evidence, with sources and file paths, is in
`src/mzoo/layers/attn/fp_attn_survey.md` and the longer `fp_attn_notes.md`.

The running example is one attention head: $S = QK^\top/\sqrt{d}$, $P = \operatorname{softmax}(S)$,
$O = PV$, with tiles of $B_M$ queries by $B_N$ keys of width $d$. Everything below is about where
each operand is quantised and what the scales cost.
...
1. **Static P scale.** $P \in [0, 1]$ and its row max is 1 under the online softmax, so
   quantise with a fixed $\delta_P = 1/448$. A *global constant* dequant means the PV mma
   accumulates directly into the output accumulator — no per-row rescale pass, no second buffer.
...
scale correction, one level deeper. Read the two levels back in words: the first level moves the
whole row into the top of the e2m1 range where its 8 magnitudes have resolution, and the second
level is the ordinary NVFP4 block scale. Cos-sim rises to 99.5%. The kernel also keeps smooth-K and
adds smooth-Q, with the rank-1 correction $Q\bar k^\top$-term added back into the score
accumulator.
...
- The forward **fake-quantises** ($\hat X = \phi^{-1}(\phi(X))$) and computes the GEMMs in **bf16**
  — the FP4 tensor-core path is not used for training at all on SM120.
...
- The STE is asymmetric by design: $dS = P \odot (dP - D)$ uses the **unquantised** $P$ while
...
2–4×, because 99 KB of shared memory forces small tiles and 273 GB/s caps everything. So the
...
FlashInfer repacks fp8/fp4 KV tiles into a 16-bit staging buffer then does bf16 `ldmatrix` + mma;
FlashMLA dequantises in registers before a bf16 UMMA; and DSV4.1's own reference kernels
dequantise the main cache rather than multiply it in FP4. The arithmetic-intensity version of the
argument: prefill is compute-bound and FP4 compute is ~FP8, so nothing is lost by bf16 math; decode
is bandwidth-bound and FP4 storage is a 3.6× byte reduction (§ one-kernel), which then transfers
almost fully to wall-clock.
...
1. A naive attention forward moves about eight times more intermediate bytes than useful bytes at
   the reference training length, and the factor grows with $S$. Every kernel decision in this
...
3. On SM120 the tile shape is derived, not chosen: a $64 \times 256$ fp32 output accumulator is
   128 registers/thread, which fixes $B_M = 64$ and leaves room for at most two 32 KB KV stages in
   99 KB of shared memory. $D = 512$ forces a split-$D$ scheme.
...
7. Decode for one query moves $\approx 128 \times 512 \times 1 + k \times 512 \times 0.5625$
   bytes per block; at the paper's $k = 512$ that is 0.21 MB against 0.64 MB if the caches were
   bf16.
...
13. On a part where FP4 mma runs at ≈FP8 speed and 273 GB/s caps everything, four bits are a
    ...
    (FlashInfer, FlashMLA, DSV4.1's own) agree; the indexer score, whose consumer only ranks, is
    the one place FP4 math pays.
