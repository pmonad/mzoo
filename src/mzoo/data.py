"""Pretokenize TinyStories into packed fixed-length blocks, saved with its tokenizer.

    just src/mzoo/ data                        # ~100M train tokens, seq_len 512
...
def prepare(tokens=100_000_000, seq_len=512, tokenizer="TinyLlama/TinyLlama-1.1B-Chat-v1.0", out=DATA, num_proc=16):
    tok = AutoTokenizer.from_pretrained(tokenizer)
    train = load_dataset("roneneldan/TinyStories", split="train")
    per_story = sum(map(len, tok(train[:1000]["text"])["input_ids"])) / 1000 + 1
    # overshoot: estimate is from a sample and packing drops remainders; extra blocks are cut below
    train = train.select(range(min(len(train), int(tokens / per_story * 1.2) + 1000)))
    ...
    ds = DatasetDict(
        ...
        validation=packed(load_dataset("roneneldan/TinyStories", split="validation"), tok, seq_len, num_proc),
    )
    ds.save_to_disk(out)
