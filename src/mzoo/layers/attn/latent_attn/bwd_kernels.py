# source: copied from ../dense_attn/bwd.py (tile-ai/tilelang@v0.1.14 examples/flash_attention/example_mha_bwd_bshd.py)
"""latent_attn: the ``preprocess`` and ``dKV`` kernels of the shared-latent MQA backward.
...
@tilelang.jit(**_JIT)
def bwd_kv(batch, heads, seq_len, dim, is_causal, splits=1, block_M=128, block_N=64, num_stages=1, threads=256):
    ...
    def main(
        ...
        with T.Kernel(splits, T.ceildiv(seq_len, block_M), batch, threads=threads) as (bx, by, bz):
            ...
            # causal: the first query tile whose last token reaches this KV block
            loop_st = T.floordiv(by * block_M, tq) if is_causal else 0
            ...
            for k in T.Pipelined(loop_st + bx * chunk, T.min(loop_ed, loop_st + (bx + 1) * chunk), num_stages=num_stages):
                ...
                if is_causal:  # query row j lives at token k * tq + j // H
                    for i, j in T.Parallel(block_M, block_N):
                        qkT[i, j] = T.if_then_else(by * block_M + i <= k * tq + j // heads, qkT[i, j], 0)
