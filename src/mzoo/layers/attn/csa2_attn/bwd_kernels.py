# source: copied VERBATIM from ../csa_attn/bwd_kernels.py (itself ../swa_attn/, ../latent_attn/,
# tile-ai/tilelang@v0.1.14 examples/flash_attention/example_mha_bwd_bshd.py)
"""csa2_attn: the ``preprocess`` and window-slice ``dKV`` kernels of the sparse backward.

**Byte-for-byte ``csa_attn``'s file** (module docstring aside), for the same reason that
let ``csa_attn`` inherit it from ``swa_attn``: the only thing the second KV source
contributes to the window slice is ``lse``, and ``lse`` is an *input* here. Gathering the
main source instead of walking it densely changes nothing about either kernel, and both
keep ``csa_attn``'s tile config, so the window ``dKV`` of this package is bit-for-bit
``csa_attn``'s / ``swa_attn``'s (pinned by ``attn_test.py``).

What is *not* inherited is the main source's KV-owner kernel (``csa_attn/bwd_main.py``):
gathered rows have no query range a group block could walk, so ``dmain_kv`` becomes a
scatter-add out of the query-owning ``bwd_dq.py``. That is the whole content of ticket 0005.

All query-side tensors are the **packed** ``[B, S*H, D]`` view of BSHD, so a query tile of
``block_N`` rows is ``T_q = block_N // H`` consecutive tokens x ``H`` heads, exactly as in
``fwd.py``. The latent is ``[B, S, D]``.

This CTA owns latent tokens ``[k0, k0 + block_M - 1]`` and only the query tiles whose
tokens can *see* them (``k <= t <= k + window - 1``), and accumulates ``dK`` and ``dV``
into **one** fp32 fragment (``dKV = dK + dV``, K == V), reduced over every head and query
token the block sees for free because heads are rows of the query tile. The grid gains a
**split** dimension over the query loop, each split writing its own fp32 slice of
``dKV_partial [splits, B, S, D]`` which torch sums -- fp32 split buffers, never bf16
atomics, and no ``dQ`` here (see ``bwd_dq.py`` and ``../swa_attn/README.md``).
"""
