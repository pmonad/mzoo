"""swa_attn: sliding-window shared-latent MQA FlashAttention-2 backward driver.
...
The one change from ``latent_attn``: both GEMM kernels walk only the ``window``
band. ``bwd_kv`` loops the query tiles whose tokens can see its latent block
(``k <= t <= k + window - 1``), ``bwd_dq`` loops the KV tiles in the band, and the
same per-row mask ``t - window < k <= t`` guards the recomputed ``P`` in both --
which is correctness, not just speed, since ``lse`` normalised the band only.
...
``CONFIGS`` is re-swept per head dim for the banded loop (see ``README.md``) and
is *not* ``latent_attn``'s table: with the band, ``bwd_kv`` wants the **smallest**
latent tile (``block_M=64``, 128 threads) rather than the largest, because a wide
KV block drags in query tiles that most of its rows cannot see. In ``bwd_kv``
``block_M`` counts latent tokens and ``block_N`` packed query rows; in ``bwd_dq``
it is the other way round, matching ``fwd.py``. Whichever of the two counts packed
query rows must be a multiple of ``H``. The tilelang layout-inference limits of
``tickets/0001-tilelang-issues.md`` still apply: ``block_M=32`` never compiles, and
64-row tiles need 128 threads.
"""

import torch

from mzoo.layers.attn.swa_attn.bwd_dq import bwd_dq
from mzoo.layers.attn.swa_attn.bwd_kernels import bwd_kv, preprocess
from mzoo.layers.attn.swa_attn.fwd import check_shapes
...
TARGET_CTAS = 768  # ~16 waves on GB10's 48 SMs; drives the query-loop split count
MAX_SPLITS = 12  # measured optimum at B1 S4096 W128; 24+ splits start losing (see README)


def pick_splits(batch: int, seq_len: int, block_M: int, window: int, tq: int,
                max_splits: int = MAX_SPLITS) -> int:
    """Enough query-loop splits to fill the GPU, capped by the band.

    Heads are rows, so the KV grid alone is tiny (``B * S/block_M``); but with the
    window each CTA only walks ``ceil((block_M + window - 1) / T_q)`` query tiles,
    so splitting further than that just launches empty CTAs.
    """
    blocks = batch * -(-seq_len // block_M)
    tiles = -(-(block_M + window - 1) // tq)
    return max(1, min(max_splits, tiles, -(-TARGET_CTAS // blocks)))


def bwd(q, kv, o, lse, do, *, window: int, causal: bool = True, sinks=None,
        cfg: dict | None = None, splits: int | None = None):
    """Sliding-window shared-latent MQA FA2 backward.

    -> ``(dq [B,S,H,D], dkv [B,S,1,D], dsinks [H] | None)``, bf16 grads. ``window``
    is the same compile-time constant the forward was called with.
    """
    assert causal, "swa_attn is causal-only"
    ...
    batch, seq_len, heads, dim = check_shapes(q, kv, dq_cfg, "swa_attn bwd (dq)")
    assert kv_cfg["block_N"] % heads == 0, (
        f"swa_attn bwd at D={dim} needs heads | {kv_cfg['block_N']} (the dKV kernel's query tile), got H={heads}")
    tq = kv_cfg["block_N"] // heads
    blk = max(kv_cfg["block_M"], tq)
    assert seq_len % blk == 0, f"swa_attn bwd needs seq_len % {blk} == 0, got {seq_len}"
    window = min(window, seq_len)
    splits = splits or pick_splits(batch, seq_len, kv_cfg["block_M"], window, tq)
    ...
    dq = bwd_dq(batch, heads, seq_len, dim, window, causal, **dq_cfg)(q, kv, do, lse, delta)
