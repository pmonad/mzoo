...
"""swa_attn: the query-owning ``dQ`` kernel of the sliding-window shared-latent MQA backward.
...
One change from ``latent_attn``: the KV loop is the same band as the forward --
tiles overlapping ``[t0 - window + 1, t0 + T_q - 1]`` -- and the recomputed ``P``
carries the same per-row band mask ``t - window < k <= t``. Without the lower edge
the recomputed ``P`` would include columns the forward never normalised against,
so the mask is correctness, not just speed.
...
def bwd_dq(batch, heads, seq_len, dim, window, is_causal, block_M=128, block_N=64, num_stages=2, threads=256):
    ...
    def main(
        ...
        with T.Kernel(T.ceildiv(seq_len, tq), batch, threads=threads) as (bx, bz):
            ...
            # same band as fwd.py: KV tiles overlapping [t0 - window + 1, t0 + tq - 1]
