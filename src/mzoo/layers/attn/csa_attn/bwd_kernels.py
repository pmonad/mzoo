# source: copied VERBATIM from ../swa_attn/bwd_kernels.py (originally ../latent_attn/, itself
# tile-ai/tilelang@v0.1.14 examples/flash_attention/example_mha_bwd_bshd.py)
"""csa_attn: the ``preprocess`` and window-slice ``dKV`` kernels of the two-source backward.

**Unchanged from ``swa_attn``.** The compressed source owns its own KV-owner kernel
(``bwd_main.py``, ``dmain_kv``) because its rows are group indices, not tokens, and
its visibility rule is group-causal rather than a band -- but the window slice's
``dKV`` is exactly the ``swa_attn`` kernel, since ``lse`` (which is all the main
source contributes to it) is an input either way.
...
As in ``swa_attn``, this CTA owns latent tokens
